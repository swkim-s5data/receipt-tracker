...
| 항목 | 내용 |
|------|------|
| OCR API | Upstage Document Parse API (`/v1/document-ai/document-parse`) |
| OCR 모델 | `document-parse-260128` (자동 선택) |
| JSON 구조화 LLM | `solar-pro` (Chat Completions API) |
| 입력 | 이미지(JPG/PNG) 또는 PDF 파일 — multipart/form-data 직접 전송 |
| 출력 | 1단계: HTML 구조화 텍스트 → 2단계: 구조화 JSON |
| 오케스트레이션 | httpx (Document Parse) + LangChain Chain + JsonOutputParser (solar-pro) |
...
| OCR API | Upstage Document Parse API | `document-parse-260128` |
| JSON 구조화 LLM | Upstage solar-pro (Chat Completions) | - |
| 이미지 처리 | Pillow | - |
| PDF 처리 | Document Parse API 직접 전송 (pdf2image 불필요) | - |
...
| 0-2 | `pdf2image` + Poppler 동작 | 샘플 PDF 변환 스크립트 실행 | 이미지 파일 정상 생성 |
| 0-3 | Vercel Python 서버리스 제약 | 공식 문서 확인 | `/tmp` 경로 사용 가능 여부 확인 |
| 0-4 | 최신 LangChain + LangChain Upstage 버전 호환성 | `requirements.txt` 의존성 확인 | 임포트 오류 없음 |
...
- [x] `pdf2image`로 샘플 PDF 변환이 로컬에서 성공한다 (Poppler `C:\poppler\poppler-24.08.0\Library\bin` 설치 완료)
- [x] Vercel 서버리스에서 `/tmp` 경로 사용 가능 여부를 공식 문서로 확인한다 (`/tmp` 500MB 사용 가능, 단 요청 간 비영속)
- [x] LangChain + ChatUpstage 조합의 이미지 전달 예제 코드를 확보한다

> **⚠️ Phase 0 아키텍처 수정사항** (2026-05-11 검증 결과)
>
> PRD에 기재된 `document-digitization-vision` 모델은 존재하지 않음. 실제 사용 가능한 API는 아래와 같음:
>
> | 항목 | 기존 PRD | 검증 후 수정 |
> |------|---------|------------|
> | OCR API | Chat Completions + `document-digitization-vision` | Document Parse API (`/v1/document-ai/document-parse`) |
> | 실제 모델명 | `document-digitization-vision` | `document-parse-260128` (자동 선택) |
> | Vision 지원 여부 | `solar-pro2`에 이미지 전달 | `solar-pro2` Vision 미지원, `solar-pro` 텍스트만 지원 |
> | PDF 처리 방식 | `pdf2image` → Base64 | Document Parse API에 PDF 직접 전송 가능 (Poppler 불필요) |
> | OCR 파이프라인 | 1단계 (LLM 단독) | **2단계** (Document Parse API → solar-pro JSON 구조화) |
> | Vercel Poppler | 바이너리 설치 예정 | Vercel에서 Poppler 바이너리 사용 불가 → Document Parse API 직접 전송으로 대체 |
...
| # | 작업 | 산출물 | 우선도 |
|---|------|--------|--------|
| 1-1 | GitHub 레포지토리 생성 및 `.gitignore` 설정 | `receipt-tracker` Repository 생성 | Must |
